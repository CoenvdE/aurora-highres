#!/usr/bin/env python3
"""Decode regional latents and plot with a global context panel.

This script reads regional latents saved by
`run_single_forward_with_latents_region.py`, decodes a single variable, and
produces a figure consisting of a global map with the region overlaid plus a
...
def _load_region_artifacts(
    latents_dir: Path,
    latents_file_name: str,
    timestamp_label: str,
    mode: str,
):
    patch_grid_path = latents_dir / "patch_grid.pt"
    latents_h5 = latents_dir / latents_file_name
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Decode regional Aurora latents and plot them with a global "
            "context panel."
        )
    )
    parser.add_argument(
