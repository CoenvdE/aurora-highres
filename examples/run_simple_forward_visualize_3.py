...
def plot_comparison_cartopy(
    ...
    Creates a 3-panel figure: Prediction, ERA5, and Error.
    """
    # Convert to Celsius
    pred_celsius = pred_data - 273.15
    era5_celsius = era5_data - 273.15
    error = pred_celsius - era5_celsius
    ...
    if region:
        ...
        if len(lat_idx) > 0 and len(lon_idx) > 0:
            ...
            error_plot = error_sorted[lat_slice, lon_slice]
            
            current_lats = lat[lat_slice]
            current_lons = lon_sorted[lon_slice]
            extent = [current_lons[0], current_lons[-1], current_lats[-1], current_lats[0]]
        else:
            pred_plot, era5_plot, error_plot = pred_sorted, era5_sorted, error_sorted
            extent = [lon_sorted[0], lon_sorted[-1], lat[-1], lat[0]]
    else:
        pred_plot, era5_plot, error_plot = pred_sorted, era5_sorted, error_sorted
        extent = [lon_sorted[0], lon_sorted[-1], lat[-1], lat[0]]
    ...
    error_max = np.abs(error_plot).max()
    ...
    data_list = [
        (pred_plot, "Aurora Prediction", 'coolwarm', vmin, vmax),
        (era5_plot, "ERA5 Ground Truth", 'coolwarm', vmin, vmax),
        (error_plot, "Error (Pred - ERA5)", 'RdBu_r', -error_max, error_max),
    ]
...
def main():
    ...
    parser.add_argument("--global-plot", action="store_true", help="Create global plot instead of regional")
    ...
    # Interpolate ERA5 to prediction grid if shapes differ
    if era5_t2m_raw.shape != pred_t2m.shape:
        print(f"  Grid mismatch: ERA5 {era5_t2m_raw.shape} vs Prediction {pred_t2m.shape}")
        print("  Interpolating ERA5 to prediction grid...")
        
        # Create interpolator (ERA5 lat is usually descending 90 to -90)
        interp = RegularGridInterpolator(
            (lat_era5[::-1], lon_era5),  # Flip lat to ascending for interpolation
            era5_t2m_raw[::-1],           # Flip data accordingly
            bounds_error=False, 
            fill_value=np.nan
        )
        
        # Create meshgrid on prediction coordinates
        lon_grid, lat_grid = np.meshgrid(lon_pred, lat_pred)
        
        # Interpolate
        era5_t2m = interp((lat_grid, lon_grid))
        print(f"  Interpolated ERA5 shape: {era5_t2m.shape}")
    else:
