...
from examples.init_exploring.helpers_plot_region import _draw_region_box
...
def load_hres_t2(device, download_path="examples/downloads/hres"):
    ...
    try:
        ...
        # Fallback to last if index 2 fails, but this is risky.
...
def plot_region_comparison(hres_data, hres_lat, hres_lon, era_data, era_lat, era_lon, region_bounds):
    ...
    def crop(data, lat, lon, bounds):
        ...
        def normalize_longitudes(lon_values):
            # Shift to [-180, 180) so western hemispheres can be selected
            return ((lon_values + 180.0) % 360.0) - 180.0

        lon_normalized = normalize_longitudes(lon)

        if lon_min <= lon_max:
            mask_lon = (lon_normalized >= lon_min) & (
                lon_normalized <= lon_max)
        else:
            # Region crosses the dateline; accept either side of the split
            mask_lon = (lon_normalized >= lon_min) | (
                lon_normalized <= lon_max)
        ...
        if lat.ndim == 1 and lon.ndim == 1:
            ...
            sub_lat = lat[lat_idx]
            sub_lon = lon_normalized[lon_idx]

            # Ensure longitudes are monotonic for plotting routines
            sort_idx = np.argsort(sub_lon)
            sub_lon = sub_lon[sort_idx]
    ...
    hres_sub, hres_lat_sub, hres_lon_sub = crop(hres_data.cpu().numpy(
    ), hres_lat.cpu().numpy(), hres_lon.cpu().numpy(), region_bounds)
    era_sub, era_lat_sub, era_lon_sub = crop(era_data.cpu().numpy(
    ), era_lat.cpu().numpy(), era_lon.cpu().numpy(), region_bounds)
    ...
    extent = [hres_lon_sub.min(), hres_lon_sub.max(),
              hres_lat_sub.min(), hres_lat_sub.max()]
    ...
    im = ax.pcolormesh(hres_lon_sub, hres_lat_sub, hres_sub,
    ...
    gl.top_labels = False
    gl.right_labels = False
    im = ax.pcolormesh(era_lon_sub, era_lat_sub, era_sub, transform=ccrs.PlateCarree(
...
def main():
    ...
    plot_region_comparison(hres_data, hres_lat, hres_lon,
